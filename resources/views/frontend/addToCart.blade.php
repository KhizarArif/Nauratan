                        ...
                        <div class="thumbnail  mt-0">
                            @if ($products[0]->productImages->count() > 1)
                                @foreach ($products[0]->productImages as $productImage)
                                    <img src="{{ asset('uploads/product/small/' . $productImage->image) }}" alt="Thumbnail "
                                        title="{{ $productImage->image }}" class="thumbnail_image"
                                        onclick="swapImage('{{ asset('uploads/product/large/' . $productImage->image) }}', '{{ $productImage->id }}')">
                                @endforeach
                            @endif
                        </div>
                        <img src="{{ asset('uploads/product/large/' . $products[0]->productImages[0]->image) }}" id="mainImage"
                            alt="Main Product Image" name="product_image" class="add_to_cart_image mb-4"
                            title="{{ $products[0]->productImages[0]->image }}">
                    </div>
                </div>
            @endif
                ...
                <div class="add_product_details">
                    ...
                    <div class="row d-flex ">
                        ...
                        <div class="col-md-8 d-flex align-items-end">
                            <a class="addToCartButton" href="javascript:void(0)"
                                onclick="addToCart('{{ $products[0]->id }}', '{{ $products[0]->title }}', '{{ $products[0]->price }}')">
                                <div>
                                    Add to cart
                                </div>
                    ...
                    <div class="addToCartBedge">
                        <img src="{{ asset('frontend_assets/img/badges.jpg') }}" alt="" class="addToCartBedgeImg">
                    </div>
...
<script>
    ...
    function swapImage(imagePath, imageId) {
        ...
        document.getElementById('selectedImageId').value = imageId; // Correctly assign imageId
    }
